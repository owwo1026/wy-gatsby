...
export default function FormWrapper({ title, titleClassName, children, className, childrenWrapperClassName, isRequired, requiredClassName, full }) {
  return (
    <div className={cn(`grid ${full ? 'md:grid-cols-8' : 'grid-cols-4'} gap-5 w-full items-center`, full ? 'md:col-span-2' : '', className)}>
      ...
      <div className={cn(`${full ? 'md:col-span-7' : 'col-span-3'}`, childrenWrapperClassName)}>{children}</div>
    </div>
  );
}
