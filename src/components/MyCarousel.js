...
const MyCarousel = ({ data, activeIndex, onClose }) => {
  ...
  return (
    <div
      id="my-carousel"
      className={`absolute top-1/2 left-1/2 transform -translate-x-1/2  overflow-hidden h-[90vh] w-full` + sm + md + lg + xl + xxl}
    >
      <button
        id="modal-close"
        type="button"
      ...
      <div className="absolute z-30 flex -translate-x-1/2 bottom-5 left-1/2 space-x-3 rtl:space-x-reverse">
        {data.map((_node, idx) => (
          <button id={`carousel-indicator-${idx}`} type="button" className="h-3 w-3 rounded-full" aria-current="true" aria-label={`Slide ${idx}`} />
        ))}
      </div>
      {/* 上一張BUTTON */}
      <button
